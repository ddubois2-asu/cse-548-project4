...
TrainingDataPath='./'
TrainingData='Training_SC.csv'
TestingDataPath='./'
TestingData='Testing_SC.csv'
# Batch Size
BatchSize=10
# Epohe Size
NumEpoch=10
...
trainingDataset = pd.read_csv(TrainingDataPath+TrainingData, header=None)
X_train = trainingDataset.iloc[:, 0:-2]
print(len(X_train[0]))
label_column = trainingDataset.iloc[:, -2].values
y_train = []
...
#Parse the Testing data set
testingDataset = pd.read_csv(TestingDataPath+TestingData, header=None)
print(type(testingDataset))
X_test = testingDataset.iloc[:, 0:-2]
print(type(X_test))
print(len(X_test[0]))
label_column = testingDataset.iloc[:, -2].values
y_test = []
for i in range(len(label_column)):
    if label_column[i] == 'normal':
        y_test.append(0)
...
#Use one hot encoder to encode the different types into integer values
...
#Get the training size so we can get the training data back after transform
trainSize = len(X_train)

#Combine the data to make sure we have all of the same inputs to the neural network
X_combined = pd.concat([X_train, X_test])
X_combined = np.array(ct.fit_transform(X_combined), dtype=np.float)
#Get training data back
X_train = X_combined[0:trainSize]
#Get testing data back
X_test = X_combined[trainSize:]
